...
const DoanhThuTheoLoaiDonHang = (props) => {
  ...
  useEffect(() => {
    if (props.isReFetchData && props.loaiBaoCao === 5) {
      ...
      reportService.getDataReportDTLoaiDH(data).then(res => {
        if (res.data) {
          setDataDetail(res.data.dtLoaiDHDetails);

          const dtLoaiDH = res.data.dtLoaiDH;
          if (dtLoaiDH) {
            var loaidH = dtLoaiDH.map((x) => {
              return x.loaiDonHang
            });

            var soTien = dtLoaiDH.map((x) => {
  ...
  const calculatorSum  = (name) => {
    ...
    return (
      ...
      <Bar
        data={{
          labels: dataReport.loaiDH,
          datasets: [
            {
              label: "Doanh thu theo loại đơn hàng",
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell sx={{ color: '#1976d2' }}>Loại đơn hàng</TableCell>
          ...
          <TableBody>
            {dataDetail && dataDetail.map((x) => {
              return <>
                ...
                >
                  <TableCell component="th" scope="row">{x.loaiDonHang}</TableCell>
...
function mapStateToProps(state) {
  ...
  export default connect(mapStateToProps)(DoanhThuTheoLoaiDonHang);