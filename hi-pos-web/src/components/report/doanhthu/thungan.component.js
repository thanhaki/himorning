...
const DoanhThuThuNgan = (props) => {
  ...
  useEffect(() => {
    if (props.isReFetchData && props.loaiBaoCao === 4) {
      ...
      reportService.getDataReportDTThuNgan(data).then(res => {
        if (res.data) {
          setDataDetail(res.data.dtThuNganDetails);

          const dtThuNgan = res.data.dtThuNgan;
          if (dtThuNgan) {
            var tn = dtThuNgan.map((x) => {
              return x.fullName
            });

            var soTien = dtThuNgan.map((x) => {
  ...
  return (
    <>
      <Bar
        data={{
          labels: dataReport.thuNgan,
          datasets: [
            {
              label: "Doanh thu theo thu ngân",
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell sx={{ color: '#1976d2' }}>Thu Ngân</TableCell>
...
export default connect(mapStateToProps)(DoanhThuThuNgan);