...
import AddPhieuKiemKe from './addPhieuKiemKe.component';
...
function PhieuKiemke(props) {
	...
	const handleDelete = () => {
		...
		if (listCheck.length > 0) {
			showMessageByType(null, "Phiếu đã kiểm kê đã nhập/hủy kho không thể xóa", TYPE_ERROR.warning)
		}
		else {
	...
	return (
		<ThemeProvider theme={theme}>
			<Container component="main" maxWidth="true">
				...
				{open && <AddPhieuKiemKe open={open} title={"THÊM MỚI PHIẾU KIỂM KÊ"} handleClose={handleClose} handleLoadPageParent={handleLoadPage} />}
				{openEdit && <AddPhieuKiemKe open={openEdit} title={nameTitle} nameRow={nameRow} handleClose={handleCloseEdit} handleLoadPageParent={handleLoadPageEdit} />}
				<AlertDialogMessage
					open={openDelete}
...
export default connect(mapStateToProps)(PhieuKiemke);