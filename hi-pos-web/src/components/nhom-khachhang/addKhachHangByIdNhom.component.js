...
import RenderIsCheckComponent from './render-check-kh.component';
...
const AddKhachHangByIdNhom = (props) => {
	...
	useEffect(() => {
		if (open || props.isReFetchData) {
			if (nameRow && Object.keys(nameRow).length > 0) {
				var data = {
					ma_TD: nameRow.row.ma_NKH,
					name: nameSearch,
				}
				setTenKhachHang(nameSearch);
				dispatch(showLoading(true));
				nhomKhachHangService.getKhachHangIsCheck(data).then((result) => {
	...
	const handleSearch = (event) => {
		...
		{
			var data = {
				ma_NKH: (nameRow && Object.keys(nameRow).length > 0 ? nameRow.row.ma_NKH : ''),
				name: tenKhachHang,
			}
			dispatch(showLoading(true));
			nhomKhachHangService.getKhachHangIsCheck(data).then((result) => {
	...
	const handleSave = () => {
		...
		var data = {
			ma_NKH: props.nameRow.row.ma_NKH,
			ten_NKH: props.nameRow.row.ten_NKH,
			ghiChu_NKH: props.nameRow.row.ghiChu_NKH,
			donVi: 0,
			ids: lstArr,
		}
		dispatch(showLoading(true));
		nhomKhachHangService.updateKhachHangByIdNhom(data).then((result) => {
			dispatch(hideLoading());
...
export default connect(mapStateToProps)(AddKhachHangByIdNhom);