...
import ProductDetail from './electronic/product-detail.component';
...
const EditProduct = (props) => {
	...
	const [selectedImageOld, setSelectedImageOld] = useState('');
	...
	useEffect(() => {
		...
		if (open) {
			if (nameRow && Object.keys(nameRow).length > 0) {
				...
				setIdDanhMuc(nameRow.row.ma_DanhMuc);
				setTenMatHang(nameRow.row.ten_MH);
				setIdLoaiMH(nameRow.row.id_LoaiMH)
				setTxtGiaBan(nameRow.row.gia_Ban);
				setTxtGiaVon(nameRow.row.gia_Von);
				setNhapGiaKhiBan(nameRow.row.isNhapGiaBan);
				setIdDonViTinh(nameRow.row.ma_DonVi);
				setMoTa(nameRow.row.mota_MH);
				setInputSoluong(nameRow.row.soLuongTonKho);
				setInputSoluongTT(nameRow.row.tonKhoMin);
				setIdLoaiBep(nameRow.row.ma_Printer === null || nameRow.row.ma_Printer === undefined ? 0 : nameRow.row.ma_Printer);
				let valCheck = nameRow.row.tonKho === true ? '1' : '0';
				setSelectedValue(valCheck);
				setSelectedImageOld(nameRow.row.hinhAnh_MH);
				setThoiGianApDung(nameRow.row.thoiGianApDung);
				setLoaiThoiGianApDung(nameRow.row.loaiThoiGianApDung);
				setQRCode(nameRow.row.qrCode);
				setSelectedValueIcon(nameRow.row.hinhAnh_MH.length > 0 ? 'image' : 'color' )
				var params = nameRow.row.ma_MH;
				thucDonMatHangService.getThucDonMhBy(params).then((result) => {
					setThucDon(result.data);

					let thucDonIsCheck = result.data.filter(x => x.isCheck);
					setTenThucDon(thucDonIsCheck);
					dispatch(hideLoading());
				}).catch((error) => {
					dispatch(hideLoading());
					showMessageByType(error, "error", TYPE_ERROR.error);
	...
	const handleUpdate = () => {
		...
		var data = {
			...
			imgOld: selectedImageOld,
			loaiThoiGianApDung: loaiThoiGianApDung,
			thoiGianApDung: thoiGianApDung,
			qRCode: qrCode
			// hinhAnh_MH: selectedImage === null ? selectedImageOld : selectedImage,
		}
		if (tenMatHang === "") {
			showMessageByType(null, "Chưa nhập tên mặt hàng", TYPE_ERROR.error)
		}
		else {
			dispatch(showLoading(true));
			let img = selectedImage === null ? selectedImageOld : selectedImage;
			const formData = new FormData();
			formData.append("file", img);
			formData.append("data", JSON.stringify(data));

			ProductService.UpdateChiTietMatHangById(formData).then((res) => {
				dispatch(hideLoading());
				showMessageByType(null, "Chỉnh sửa mặt hàng thành công", TYPE_ERROR.success);
				handleCancel();
			}).catch((error) => {
				showMessageByType(error, "Chỉnh sửa mặt hàng không thành công", TYPE_ERROR.error)
				dispatch(hideLoading());
			})
		}
	}
	...
	const HandleSearchThucDon = (event) => {
		...
		setTenThucDon(typeof value === 'string' ? value.split(',') : value);
	};
			...
			>
				<DialogContent>
					...
					<Grid item sx={12} sm={12} >
						<DialogTitle id="scroll-dialog-title">Chỉnh sửa mặt hàng</DialogTitle>
					</Grid>
					<Grid container spacing={1} style={{ padding: 10 }}>
						<Grid item sx={12} sm={8}>
							<Grid container spacing={1}>
								...
								<Grid item xs={12} sm={2}>
									<Button onClick={() => setOpenProductDetail(true)} style={{ fontSize: 12 }}>Miêu tả thêm</Button>
								</Grid>
							</Grid>
						</Grid>
						<Grid item sx={12} sm={4}>
							<Grid container spacing={1}>
								...
								<Grid container xs={12} sm={6}>
									...
									<Grid item xs={12} sm={10} sx={{ marginTop: 1 }}>
										<FormHelperText>Hình ảnh (225px x 225px)</FormHelperText>
									</Grid>
								</Grid>
								<Grid item>
									{selectedValueIcon === 'image' && (
										<ImageUpload
											...
											urlImage={nameRow.row.hinhAnh_MH}
					...
					<ProductDetail
						...
						ma_MH={nameRow.row.ma_MH}
						handleClose={() => setOpenProductDetail(false)}
					/>
				</DialogContent>
			</Dialog>
